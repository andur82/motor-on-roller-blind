...
void NidayandHelper::mqtt_reconnect(PubSubClient& psclient, String uid, String pwd, std::list<const char*> topics){
  ...
  while (!psclient.connected()) {
    ...
    // Attempt to connect
    if ((mqttLogon ? psclient.connect(this->_mqttclientid.c_str(), uid.c_str(), pwd.c_str(), mqtt_gettopic("LWT").c_str(), 0, 1, "Offline") : psclient.connect(this->_mqttclientid.c_str(), mqtt_gettopic("LWT").c_str(), 0, 1, "Offline"))) {
      ...
      this->mqtt_publish(psclient, mqtt_gettopic("LWT"), "Online");
      //Setup subscription
      if (!topics.empty()){
